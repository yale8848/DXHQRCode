	...
	public void onDraw(Canvas canvas) {
		...
		if (resultBitmap != null) {
			...
			canvas.drawRect(frame.right - L_WIDTH, frame.top, frame.right,
					frame.bottom, paint);
			// 画扫描框边上的角，总共8个部分
			paint.setColor(getContext().getResources().getColor(R.color.viewfinder_rect_corner_blue));
			...
			if ((CaptureActivity.currentState != null)
				...
				canvas.drawText(text, getWidth() / 2 - paint.measureText(text)
						/ 2, (float) (frame.bottom + (float) TEXT_PADDING_TOP
						* density), paint);
